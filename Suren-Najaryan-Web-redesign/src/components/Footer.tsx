...
export function Footer() {
  ...
  return (
    <footer className="sn-footer sn-footer-minimal">
      <div className="sn-footer-action">
        <h2>První krok je krátká zpráva.</h2>
      ...
      <div className="sn-footer-details">
        <a href={`tel:${siteConfig.phoneHref}`}>Tel: {siteConfig.phone}</a>
        <span>IČ: {siteConfig.ico}</span>
        <span>{siteConfig.location}, {siteConfig.locationDetail}</span>
        <span>© {new Date().getFullYear()} {siteConfig.name}. Všechna práva vyhrazena.</span>
      </div>
    </footer>
  );
}
